<template>
  <div class="container">
    ...
    >
      ...
      <vxe-column field="car_type" title="汽车类型" :edit-render="{}">
        <template #edit="{ row }">
          <vxe-input v-model="row.car_type" type="text" placeholder="汽车类型" />
        </template>
      </vxe-column>
      ...
      <vxe-column field="driver_phone" title="司机电话" :edit-render="{}">
        <template #edit="{ row }">
          <vxe-input v-model="row.driver_phone" type="text" placeholder="司机电话" />
        </template>
      </vxe-column>
      <vxe-column field="illegal_time" title="违章时间" :edit-render="{}">
        <template #edit="{ row }">
          <vxe-input
            v-model="row.illegal_time"
            type="date"
            placeholder="请选择日期"
            transfer
          />
      ...
      <vxe-column field="illegal_location" title="违章地点" :edit-render="{}">
        <template #edit="{ row }">
          <vxe-input v-model="row.illegal_location" type="text" placeholder="违章地点" />
        </template>
      </vxe-column>
      <vxe-column field="illegal_behaviour" title="违章行为" :edit-render="{}">
        <template #edit="{ row }">
          <vxe-input v-model="row.illegal_behaviour" type="text" placeholder="违章行为" />
        </template>
      </vxe-column>
      <vxe-column field="illegal_deduct" title="本次扣分" :edit-render="{}">
        <template #edit="{ row }">
          <vxe-input v-model="row.illegal_deduct" type="text" placeholder="本次扣分" />
        </template>
      </vxe-column>
      <vxe-column field="illegal_fee" title="罚金" :edit-render="{}">
        <template #edit="{ row }">
          <vxe-input v-model="row.illegal_fee" type="text" placeholder="罚金" />
        </template>
      </vxe-column>
...
export default {
  ...
  methods: {
    ...
    searchEvent: _.debounce(function() {
      ...
      if (filterName) {
        const searchProps = ['name', 'car_type', 'driver', 'driver_phone', 'illegal_location', 'illegal_behaviour', 'illegal_deduct', 'illegal_time', 'illegal_fee']
        const rest = this.illegalList.filter(item => searchProps.some(key => XEUtils.toValueString(item[key]).toLowerCase().indexOf(filterName) > -1))
